...
class VXRecognitionModelAutoML: VXRecognitionModelProtocol {
    ...
    public func isModelOnDisk() -> Bool {
        guard let modelName = detectionModel else {
            return false
        }
    ...
    public func load(completion: @escaping (Bool) -> Void) {
        ...
        if let mm = modelManager, mm.isRemoteModelDownloaded(remoteModel), self.isModelOnDisk() {
            ...
            downloadProgress = modelManager?.download(remoteModel)
